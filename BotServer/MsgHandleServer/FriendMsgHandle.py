...
class FriendMsgHandle:
    ...
    def mainHandle(self, msg):
        ...
        if msg.type == 3:  # 图片消息
            ...
            op(f"- 发送者: {sender}")
            op(f"- 消息ID: {msg.id}")
            op(f"- Extra: {msg.extra}")
            try:
                self.wcf.image_cache.record_image(sender, sender, msg.id, msg.extra)
            except Exception as e:
                op(f"[Error] 记录图片信息失败: {e}")
                return
        ...
        # 红包消息处理 转发红包消息给主人
        if msg.type == 10000 and '请在手机上查看' in msg.content:
            Thread(target=self.forwardRedPacketMsg, args=(sender,)).start()
        # 好友自动同意处理 暂时没用 老版本微信有用
        if msg.type == 37 and self.acceptFriendLock:
    ...
    def getAiMsg(self, content, sender):
        ...
        try:
            # 获取图片OCR结果
            ocr_result = self.wcf.image_cache.get_image_ocr(self.wcf, sender, sender)
            combined_msg = content
            
            if ocr_result:
                combined_msg = f"{content}\n图片内容：{ocr_result}"
                op(f"[Debug] 最终发送给AI的内容: {combined_msg}")
            
            # 不管是否有图片，都发送消息给AI
            aiMsg = self.Ad.getAi(combined_msg)
            if aiMsg:
                self.wcf.send_text(aiMsg, receiver=sender)
                return
                
            self.wcf.send_text('Ai对话接口出现错误, 请联系超管查看控制台输出日志', receiver=sender)
            
        except Exception as e:
            op(f"[Error] 处理图片OCR失败: {e}")
            self.wcf.send_text('处理图片失败, 请联系超管查看控制台输出日志', receiver=sender)
